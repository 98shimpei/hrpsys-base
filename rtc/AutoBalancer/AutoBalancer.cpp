...
hrp::Matrix33 mat_ave (const hrp::Matrix33& a, const hrp::Matrix33& b)
{
  ...
  ans <<
    ...
    (tmp * hrp::Vector3::UnitY()).normalized(),
    (tmp * hrp::Vector3::UnitZ()).normalized();
  return ans;
}
...
RTC::ReturnCode_t AutoBalancer::onInitialize()
{
    ...
    hand_fix_body_gain = hrp::Vector3(0, 0, 0);
  ...
  // std::cerr << "AutoBalancer::onExecute(" << ec_id << ")" << std::endl;
    ...
    if (m_boxPoseIn.isNew()) {
      ...
      for (int i = 0; i < m_boxPose.data.poses.length(); i++){
        ...
        hrp::Matrix33 tmp_rot = tmp_quat.matrix();
...
//        updateInvDynStateBuffer(idsb);
    ...
    if (is_legged_robot) {
      ...
      m_shimpei.data[12] = xrefddk->getCurrentValue()(0);
      m_shimpei.data[13] = xrefddk->getCurrentValue()(1);
      m_shimpei.data[14] = xrefddk->getCurrentValue()(2);
    ...
    }else if(st->control_mode == Stabilizer::MODE_ST) {
      tmp_ratio = 1.0;
    }else{
      tmp_ratio = 0.0;
    }
...
void AutoBalancer::updateTargetCoordsForHandFixMode (coordinates& tmp_fix_coords)
{
    ...
    if (st->box_control_mode) {
      ...
      w3->passFilter(hrp::Vector3(0, 0, 0));
    }
    limit->passFilter(hand_fix_slip_limit);
    hrp::Vector3 center = m_robot->link("CHEST_JOINT0")->p + m_robot->link("CHEST_JOINT0")->R*hrp::Vector3(0.65, 0, 0);
...
*/
    ...
    ikp["rarm"].target_p0 += hrp::Vector3(0, 0, jamp_box_amp * (-std::pow((jamp_box_angle - M_PI) / M_PI, 6) + 3 * std::pow((jamp_box_angle - M_PI) / M_PI, 4) - 3 * std::pow((jamp_box_angle - M_PI) / M_PI, 2) + 1));
    ikp["larm"].target_p0 += hrp::Vector3(0, 0, jamp_box_amp * (-std::pow((jamp_box_angle - M_PI) / M_PI, 6) + 3 * std::pow((jamp_box_angle - M_PI) / M_PI, 4) - 3 * std::pow((jamp_box_angle - M_PI) / M_PI, 2) + 1));
...
void AutoBalancer::updateWalkingVelocityFromHandError (coordinates& tmp_fix_coords)
{
    ...
    if ( gg_is_walking && gg->get_lcg_count() == gg->get_overwrite_check_timing()+2 ) {
        ...
        if (graspless_manip_mode) {
          ...
          if (vel_htc(0) > 0.12) vel_htc(0) = 0.12;
          else if (vel_htc(0) < -0.07) vel_htc(0) = -0.07;
          if (vel_htc(1) > 0.05) vel_htc(1) = 0.05;
          else if (vel_htc(1) < -0.05) vel_htc(1) = -0.05;
          if (vel_htc(2) > 15.0) vel_htc(2) = 15.0;
          else if (vel_htc(2) < -15.0) vel_htc(2) = -15.0;
...
void AutoBalancer::solveFullbodyIK ()
{
  ...
  limbStretchAvoidanceControl();
    ...
    }
    {
        ...
        if (ikp.size() >= 4 && (ikp["rarm"].is_active || ikp["larm"].is_active)) normal_ratio = 1e-6; //1e-6
...
bool AutoBalancer::setAutoBalancerParam(const OpenHRP::AutoBalancerService::AutoBalancerParam& i_param)
{
  ...
  if (!gg_is_walking) {
      ...
      for (int i = 0; i < 3; i++) {
        hand_fix_body_gain[i] = i_param.hand_fix_body_gain[i];
      }
